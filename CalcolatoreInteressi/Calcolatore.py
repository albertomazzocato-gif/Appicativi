...
def inizializza_storico():
    if not os.path.exists(EXCEL_STORICO):
        ...
        ws.append(["Data Calcolo", "Cliente", "Capitale Iniziale (€)", "Data Inizio", "Data Fine", "Totale Interessi (€)", "Totale Rivalutato (€)"])
...
st.markdown("<h2 style='color: #1b4f72; text-align: center;'>CALCOLATORE LEGALE: INTERESSI & RIVALUTAZIONE</h2>", unsafe_allow_html=True)
st.write("")
...
if submit_btn:
    if not cliente or not cap_str or not data_inizio_str or not data_fine_str or not tasso_str:
        ...
        try:
            ...
            st.error("Controlla che i numeri siano validi e le date siano nel formato GG/MM/AAAA (es: 01/01/2022).")
            st.stop()
        ...
        if d_inizio >= d_fine:
            ...
            try:
                ...
                ws = wb.active
                ws.append([datetime.now().strftime("%d/%m/%Y %H:%M"), cliente, capitale, data_inizio_str, data_fine_str, round(interessi, 2), round(totale_dovuto, 2)])
                wb.save(EXCEL_STORICO)
            except Exception as e:
                st.warning(f"Errore salvataggio Excel: {e}")
