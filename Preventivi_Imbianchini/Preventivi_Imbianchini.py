...
def genera_pdf(data, cliente, tipo_lavoro, mq, materiale_desc, costo_materiale, mano_opera, costo_totale):
    ...
    pdf.cell(45, 8, ' Quantita / Unita', 1, 0, 'C', fill=True)
...
def main():
    ...
    with col2:
        prezzo_mq = st.number_input("Prezzo al mq (EUR)", min_value=0.0, value=5.0, step=0.5)
        
    materiale_desc = st.text_input("Descrizione Materiali Usati", value="Idropittura lavabile")
    costo_materiale = st.number_input("Costo Totale Materiali (EUR)", min_value=0.0, value=150.0, step=10.0)
